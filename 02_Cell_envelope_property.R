...
residuals <- (aov(permeability ~ group, data = d_CEF_2))$residuals
shapiro.test(residuals)
bartlett.test(permeability ~ group, data = d_CEF_2)
aov.1 <- aov(permeability ~ group, data = d_CEF_2)
summary(aov.1)
TukeyHSD(aov.1)

ggplot(d_CEF_2, aes(group, permeability, fill = group))+
  theme_bw()+
  scale_y_continuous(limits = c(0, 1.5))+
  geom_jitter(aes(group, permeability), width = 0.22, 
...
residuals <- (aov(hydrophobicity ~ group, data = d_CEF_2))$residuals
shapiro.test(residuals)
bartlett.test(hydrophobicity ~ group, data = d_CEF_2)
aov.2 <- aov(hydrophobicity ~ group, data = d_CEF_2)
summary(aov.2)
TukeyHSD(aov.2)

ggplot(d_CEF_2, aes(group, hydrophobicity, fill = group))+
  theme_bw()+
  scale_y_continuous(limits = c(0, 1.25))+
  geom_jitter(aes(group, hydrophobicity), width = 0.22, 
              size = 4.5, shape = 21, stroke = 0.01, alpha = 1)+
  stat_summary(geom = "col", fun = "mean",
               position = "dodge", width = 0.625, alpha = 0.7)+
  stat_summary(geom = "errorbar",
...
residuals <- (aov(permeability ~ group, data = d_TET_2))$residuals
shapiro.test(residuals)
bartlett.test(permeability ~ group, data = d_TET_2)
aov.3 <- aov(permeability ~ group, data = d_TET_2)
summary(aov.3)
TukeyHSD(aov.3)

ggplot(d_TET_2, aes(group, permeability, fill = group))+
  theme_bw()+
...
residuals <- (aov(hydrophobicity ~ group, data = d_TET_2))$residuals
shapiro.test(residuals)
bartlett.test(hydrophobicity ~ group, data = d_TET_2)
aov.4 <- aov(hydrophobicity ~ group, data = d_TET_2)
summary(aov.4)
TukeyHSD(aov.4)

ggplot(d_TET_2, aes(group, hydrophobicity, fill = group))+
  theme_bw()+
  scale_y_continuous(limits = c(0, 1.25))+
  geom_jitter(aes(group, hydrophobicity), width = 0.22, 
              size = 4.5, shape = 21, stroke = 0.01, alpha = 1)+
