...
shapiro.test(d1$cellular)
bartlett.test(cellular ~ group, data = d1)
t.test(cellular ~ group, data = d1, var.equal = TRUE)

ggplot(d1, aes(group, cellular, fill = group))+
  theme_bw()+
  ...
  labs(x = NULL,
       y = "Cellular TET accumulation (ng"~cell^"-1"*")",
       title = "WT")+
...
shapiro.test(d2$cellular)
bartlett.test(cellular ~ group, data = d2)
t.test(cellular ~ group, data = d2, var.equal = TRUE)

ggplot(d2, aes(group, cellular, fill = group))+
  theme_bw()+
  theme(panel.border = element_rect(fill = NA, color = "black", size = 2, linetype = "solid"))+
  geom_violin(draw_quantiles = 0.1, aes(fill = group), width = 0.9, size = 1.5, alpha = 0.75)+
  geom_boxplot(width = 0.25, height = 0.5, size = 1, alpha = 0.7, outlier.shape = NA)+
